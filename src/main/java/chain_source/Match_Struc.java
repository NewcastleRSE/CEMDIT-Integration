...
/**
 * Structure that is being used to store the results from SPSM and matches.
 * 
 * Represents one match between a source schema and a target schema, with its
 * similarity score and the repaired schema and the SPARQL query that is
 * generated from the repaired schema
 * 
 * This structure will have a similarity value, the name of the target, a list
 * of the individual matches and relations as well as storing how many
...
public class Match_Struc {
	...
	private String query; // The query - either the initial query, or null if the original schema wont
							// parse, or created from the repaired schema
	private String querySchema; // The original query schema
	private String querySchemaHead; // The head of the original query schema
	

	/**
	 * Constructor of Match_Struc with params
	 * 
	 * @param sim
	 * @param targetSchema
	 */
	...
	/**
	 * Constructor of Match_Struc without param
	 * 
	 */
	public Match_Struc() {
		similarity = 0;
